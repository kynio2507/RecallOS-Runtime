...
`recallos-runtime` có hai loại log:
...
| `query` | info | `recall_runtime_query` được gọi |
| `remember` | info | `recall_runtime_remember` lưu item |
| `decision` | info | `recall_runtime_decision` lưu decision |
| `bug` | info | `recall_runtime_bug` lưu bug |
...
```text
[recallos-runtime] FATAL: cannot open DB ...
[recallos-runtime] WARN: CodeGraph not available
```
...
**Triệu chứng:** Lần đầu gọi `recall_runtime_query` với CodeGraph, phản hồi chậm 5–30s.
...
**Triệu chứng:** Đôi khi `recall_runtime_query` với `includeImpact: true` trả về kết quả trống.
...
| `0.2.0` | Thêm `recall_runtime_decision`, `recall_runtime_bug`; schema v1; CodeGraph hybrid query |
| `0.1.0` | MVP: `recall_runtime_query`, `recall_runtime_remember`, `recall_runtime_status`; SQLite DB |
