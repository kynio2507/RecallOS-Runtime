...
| `query` | info | `recall_kb_query` được gọi |
| `remember` | info | `recall_kb_remember` lưu item |
| `decision` | info | `recall_kb_decision` lưu decision |
| `bug` | info | `recall_kb_bug` lưu bug |
...
**Triệu chứng:** Lần đầu gọi `recall_kb_query` với CodeGraph, phản hồi chậm 5–30s.
...
**Triệu chứng:** Đôi khi `recall_kb_query` với `includeImpact: true` trả về kết quả trống.
